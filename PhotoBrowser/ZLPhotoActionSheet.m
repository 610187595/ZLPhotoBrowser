...
typedef void (^handler)(NSArray<ZLSelectPhotoModel *> *selectPhotos);
...
- (void)showWithSender:(UIViewController *)sender animate:(BOOL)animate completion:(void (^)(NSArray<ZLSelectPhotoModel *> *))completion
{
...
- (void)done
{
    if (self.handler) {
        ...
        self.handler(_arraySelectPhotos);
    }
}
...
- (void)cell_btn_Click:(UIButton *)btn
{
    ...
    if (btn.selected) {
        ...
        for (ZLSelectPhotoModel *smodel in _arraySelectPhotos) {
            if (smodel.asset.mediaType!=asset.mediaType) {
                ShowToastLong(@"图片和视频只能选择一种类型");
                btn.selected = !btn.selected;
                return;
            }
        }

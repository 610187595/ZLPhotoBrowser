...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        [[ZLPhotoTool sharePhotoTool] requestImageForAsset:asset size:  PHImageManagerMaximumSize resizeMode:PHImageRequestOptionsResizeModeFast completion:^(UIImage *image) {
        NSLog(@"%@",image);
        cell.imageView.image = image;
            [cell hideIndicator];
        }];
...
- (void)videoClick:(UIButton *)btn
{
    ...
    [[PHCachingImageManager defaultManager] requestPlayerItemForVideo:asset options:option resultHandler:^(AVPlayerItem * _Nullable playerItem, NSDictionary * _Nullable info) {
        NSLog(@"%@",playerItem.asset);
        dispatch_async(dispatch_get_main_queue(), ^{
            self.wmPlayer=[[WMPlayer alloc]initWithFrame:CGRectMake(0,0,self.view.frame.size.width,self.view.frame.size.height) videoURLStr:nil WithPlayerItem:playerItem];
            self.wmPlayer.canleClick=^(BOOL isYes)
            {
                [wSelf showStatusBarAndNavBar];
            };
